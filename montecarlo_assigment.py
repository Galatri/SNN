# -*- coding: utf-8 -*-
"""MonteCarlo Assigment

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1KPw4fFkM9WUaZ8Flm8otjZmgnE2TozaS
"""
...
#      The state with s=+1 has energy 1 but we allow for degeneracy
def mcmove(config, beta, degeneracy=2):
    '''Monte Carlo move using Metropolis algorithm with degeneracy in excited state '''
    for i in range(N):
        for j in range(N):
                ...
                if (s<0):
                   ...
                   #cost of decay
                   cost=-1.0
                # If in excited state, handle degeneracy by picking randomly from degenerate states
                if s == 1:
                   cost = np.random.choice([1.0] * degeneracy + [-1.0])  # Degenerate excited state
...
def calcEnergy(config, degeneracy=2):
    ...
    for i in range(len(config)):
        for j in range(len(config)):
            State = config[i,j]
            if (State>0):
              # For degenerate excited states, energy is 1 (but can be multiple excited states)
                energy += np.random.choice([1.0] * degeneracy + [-1.0])  # Degenerate excited state
            else:
                energy += 0  # Ground state has energy 0
    return energy
